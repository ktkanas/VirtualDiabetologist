...
| **Actual Result**   | Response 200 received, user data returned in JSON |
| **Status**          | ✅ **Pass** |
...
| **Actual Result**   | 401 Unauthorized response received |
| **Status**          | ❌ **Fail** (Handled correctly but token refresh mechanism absent) |
...
| **Actual Result**   | 403 Forbidden returned |
| **Status**          | ❌ **Fail** (Correct error code but unclear message content) |
...
| **Actual Result**   | 200 OK, response: `{ "status": "success", "result": "No DR" }` |
| **Status**          | ✅ **Pass** |
...
| **Actual Result**   | 400 Bad Request: `"Unsupported file type"` |
| **Status**          | ❌ **Fail** (Handled but message format inconsistent with others) |
...
| **Actual Result**   | 422 Unprocessable Entity: `"No file field in form-data"` |
| **Status**          | ❌ **Fail** (Correct error code, needs better client-side check) |
